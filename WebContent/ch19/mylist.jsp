...
<c:if test="${count > 0}">
   ...
   <c:if test="${startPage > 10}">
        <a href="/Jsp/ch19/myList.do?pageNum=${startPage - 10 }">[이전]</a>
   </c:if>

   <c:forEach var="i" begin="${startPage}" end="${endPage}">
       <a href="/Jsp/ch19/myList.do?pageNum=${i}&writer=${writer}">[${i}]</a>
   </c:forEach>

   <c:if test="${endPage < pageCount}">
        <a href="/Jsp/ch19/myList.do?pageNum=${startPage + 10}">[다음]</a>
   </c:if>
</c:if>
</center>
</body>
