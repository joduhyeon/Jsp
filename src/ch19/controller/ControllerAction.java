...
public class ControllerAction extends HttpServlet {
    ...
    throws ServletException, IOException {
		...
		try {
            String command = request.getRequestURI();   //uri를 뽑아낸다
            if (command.indexOf(request.getContextPath()) == 0) { //ContextPath가 /Jsp를 뽑아냄 indexOf 지정된 문자의 최초위치
               command = command.substring(request.getContextPath().length());
               //substring 은 ContextPath().length() 만큼  /Jsp (4글자) 를 잘라냄
            }
            com = (CommandAction)commandMap.get(command);    //CommandAcion은 인터페이스 
